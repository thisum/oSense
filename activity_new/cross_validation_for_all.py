...
def prepare_per_person_files(lbl_list, index_to_split):
        ...
        for i, filename in enumerate(files):
            ...
            for lbl in lbl_list:
                ...
                if index_to_split == 0:
                    ...
                    split_index = np.floor(raw_data.shape[0] / 4 * 3 - 1)

                    train = train.append(raw_data.loc[:split_index, :])
                    test = test.append(raw_data.loc[split_index + 1:, :])

                elif index_to_split == 1:

                    split_index = np.floor(raw_data.shape[0] / 4 * 1 - 1)
                    ...
                    split_index1 = np.floor(raw_data.shape[0] / 4 * 1 - 1)
                    split_index2 = np.floor(raw_data.shape[0] / 4 * 2 - 1)
                ...
                elif index_to_split == 3:

                    split_index1 = np.floor(raw_data.shape[0] / 4 * 2 - 1)
                    split_index2 = np.floor(raw_data.shape[0] / 4 * 3 - 1)
...
def prepare_all_user_files():
    ...
    filelist = [ f for f in os.listdir("../data/activity/txt_files/all_users_train_test/")]
    for f in filelist:
        os.remove(os.path.join("../data/activity/txt_files/all_users_train_test/", f))
    ...
    train.to_csv("../data/activity/txt_files/all_users_train_test/all_train.txt", index=False, header=False)
    test.to_csv("../data/activity/txt_files/all_users_train_test/all_test.txt", index=False, header=False)
...
def run_algorithm_all_users():

    test_df = pd.read_csv("../data/activity/txt_files/all_users_train_test/all_test.txt", header=None)
    train_df = pd.read_csv("../data/activity/txt_files/all_users_train_test/all_train.txt", header=None)

    evaluate_algorithms(train_df, test_df)
...
for w in window_list:
    ...
    for l in total_list:
        ...
        prepare_per_person_files(l, 0)
        prepare_all_user_files()
        run_algorithm_all_users()

        print("***********************************************************")

        print("***************** 1 fold *****************")
        prepare_test_train_files(l, w)
        prepare_per_person_files(l, 1)
        prepare_all_user_files()
        run_algorithm_all_users()
