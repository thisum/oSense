...
def calculate_features(s_d, f_d, prev_freq, lbl, target_file):
    ...
    appened_to_file("../data/accel_features/" + target_file + ".txt", str(feature_row), True)

    return fft_features[0]


def process_file(file_path, target_file):
    ...
    for lbl in lbl_list:
        ...
        for j in range(3):
            ...
            for i in range(int(2 * len(y) / N)):

                begin = i * 50
                if begin + N > len(y):
                    break
                series = y[begin: begin + N]
...
filelist = [ f for f in os.listdir("../data/accel_features/")]
for f in filelist:
    os.remove(os.path.join("../data/accel_features/", f))

for root, dirs, files in os.walk("/Users/thisum/Documents/Personel_Docs/NUS_MSc/Research/MagicHand_Data/sequence_data/experiment_data/processed/accel_character/raw/"):
    for i, filename in enumerate(files):
        ...
        print(filename)
        process_file(root + "/" + filename, filename.split(".")[0])
        print("\n----------------\n\n")




# raw_data = pd.read_csv("/Users/thisum/Documents/Personel_Docs/NUS_MSc/Research/MagicHand_Data/sequence_data/data/thisum/z_accel_data.txt", header=None)
# none_data = raw_data[raw_data[3].isin(['none'])]
# not_none_data = raw_data[~raw_data[3].isin(['none'])]
